...
def getData():
    ...
    sheet_props = sheet.get(spreadsheetId=spreadSheetId, fields="sheets.properties").execute()
    sheet_names = [ sheet_prop["properties"]["title"] for sheet_prop in sheet_props["sheets"]]
    sheet_names = [ sheet_name for sheet_name in sheet_names if sheet_name.find("RFS") >= 0]

    data = {}
    for sname in sheet_names:
        rows = sheet.values().get(spreadsheetId=spreadSheetId, range=sname).execute().get('values', [])
        data[sname] = rows
    return data

# def credSetup2():
#     creds = ServiceAccountCredentials.from_json_keyfile_name("serviceaccount.json", SCOPES)
